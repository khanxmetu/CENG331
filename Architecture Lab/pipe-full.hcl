...
# ID: 2650158
# Modifications #
# Added a custom cmpq operator defined as follows:
#      cmpq rA, rB:
#          if R[rA] == 0:    M[R[rB]] = '0'
#          if R[rA] != 0:    M[R[rB]] = '1'
#
...
#
# Decode Stage #
# # Try Forwarding from Execute Stage if possible otherwise fetch from register file
# valA <- if R[rA] == 0: 48
#         else: 49
#
# valB <- R[rB]
...
word d_valA = [
	...
	D_icode == ICMPQ && d_srcA == e_dstE && e_valE!=0: 49;
	D_icode == ICMPQ && d_srcA == e_dstE && e_valE==0: 48;
	D_icode == ICMPQ && d_rvalA!=0: 49;
	D_icode == ICMPQ && d_rvalA==0: 48;
...
bool mem_write = M_icode in { IRMMOVQ, IPUSHQ, ICALL, ICMPQ };
